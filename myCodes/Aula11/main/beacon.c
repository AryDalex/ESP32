...
static void ble_gap_event_handler(esp_gap_ble_cb_event_t event, esp_ble_gap_cb_param_t *param)
{
    switch (event)
    {
    case ESP_GAP_BLE_ADV_DATA_RAW_SET_COMPLETE_EVT:  // setando advertising
    ...
    case ESP_GAP_BLE_ADV_START_COMPLETE_EVT: // iniciando advertising
...
void init(void)
{
    nvs_flash_init(); // start nvs antes de usar bluetooth 
    esp_bt_controller_mem_release(ESP_BT_MODE_CLASSIC_BT); // nao esta usando, libera memoria

    esp_bt_controller_config_t bt_cfg = BT_CONTROLLER_INIT_CONFIG_DEFAULT();
    esp_bt_controller_init(&bt_cfg); // inicia controlador
    esp_bt_controller_enable(ESP_BT_MODE_BLE); // usa o esse modo bluetooth 

    esp_bluedroid_init(); // inicia
    esp_bluedroid_enable(); // abilita 

    esp_ble_gap_register_callback(ble_gap_event_handler); // pre coneccao
}
...
void app_main(void)
{
    ...
    #ifdef SCAN_RSP
    uint8_t len = strlen(name); // nome do bluetooth
    if (len > 29)
        len = 29;

    scan_rsp[0]= len + 1;
    scan_rsp[1] = 0x09;
    sprintf(&scan_rsp[2], "%.*s", len, name); // manda o nome
    #endif

    // faz advertising em "sizeof(ibeacon_adv_data)"
